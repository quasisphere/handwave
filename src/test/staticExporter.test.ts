...
test("builds a self-contained static theorem explorer without workspace path leaks", async () => {
  ...
  try {
    ...
    assert.equal(site.declarationCount, 3);
    assert.equal(site.theoremCount, 3);
    assert.equal(site.articleCount, 1);
    ...
    assert.match(
      site.html,
      /function historyRootTheoremName\(\) \{[\s\S]*?searchSelection\?\.type !== "theorem"[\s\S]*?searchSelection\.value/
    );
    assert.match(
      site.html,
    ...
    assert.match(site.html, /previewHtml \+ renderViewerInfo\(theorem\)/);
    ...
    assert.match(site.html, /"#theorem=" \+ encodeURIComponent\(state\.selectedName\)/);
